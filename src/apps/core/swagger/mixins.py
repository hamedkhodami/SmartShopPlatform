...
class SwaggerViewMixin:
    ...
    def _initial_swagger(cls):
        ...
        assert isinstance(
            cls.swagger_title, str
        ), "`swagger_title` field must be string not %s" % type(cls.swagger_title)
        ...
        additional_context = {}
        if view == _get_view:
            additional_context["query_serializer"] = (
                cls.swagger_serializer or cls.serializer
            )
        else:
            additional_context["request_body"] = (
                cls.swagger_serializer or cls.serializer
            )

        swagger_schema(
            operation_id=cls.swagger_title,
            operation_description=cls.swagger_description,
            security=cls._security,
            responses=cls._responses,
            tags=cls.swagger_tags,
            **additional_context,
        )(view)
